import mdtraj as md
traj=md.load('protein_full.xtc', top='protein.prmtop')
...
f=open('atp_A_min_dist_full.dat','w')
...
dist=md.compute_contacts(traj,contacts=pair,scheme='closest')[0]
dist_min=dist.min(axis=1)
f=open('atp_B_min_dist_full.dat','w')
for i in range(len(dist_min)):
    f.write('{} {}\n'.format(i+1, dist_min[i]*10))
f.close()
...
dist=md.compute_contacts(traj,contacts=pair,scheme='closest')[0]
dist_min=dist.min(axis=1)
f=open('fad_A_min_dist_full.dat','w')
for i in range(len(dist_min)):
    f.write('{} {}\n'.format(i+1, dist_min[i]*10))
f.close()
...
dist=md.compute_contacts(traj,contacts=pair,scheme='closest')[0]
dist_min=dist.min(axis=1)
f=open('fad_B_min_dist_full.dat','w')
for i in range(len(dist_min)):
    f.write('{} {}\n'.format(i+1, dist_min[i]*10))
f.close()
